...
marker_gene_file="02_promoter_coordinates.txt"
...
for i in $(seq 0 $((num_genes-1)) ); do
	...
	# create directory
	mkdir -p promoter_only/$gene_name

	# generate a bigwig file for each cell type
	for j in $(seq 0 $((num_genes-1)) ); do
		cell_type=${cell_types[j]}
		echo "$cell_type"
		bigWigToBedGraph -chrom=$chr -start=$start_pos -end=$end_pos ../cell_type_bigwig/$cell_type.bw promoter_only/"$gene_name"/"$cell_type"_"$gene_name".bedGraph
	done
done
