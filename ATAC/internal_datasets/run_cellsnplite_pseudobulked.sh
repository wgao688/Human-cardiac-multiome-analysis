...
# Run cellsnp-lite with the specified arguments to do it at a single cell level -- this takes much longer, so do not do this
...
# run without UMItag since this is ATAC
cellsnp-lite -s $bam_file -O $output_dir -p $threads --minMAF $min_maf --minCOUNT $min_count --cellTAG None --UMItag None --gzip
