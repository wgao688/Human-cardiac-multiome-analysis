...
process_dir() {
	...
	#echo "bash run_STARSolo_human_heart.sh -g $GENOME_DIR -t $THREADS -1 $FASTQ_R1 -2 $FASTQ_R2 -v $TECHNOLOGY -o STAR"
	bash run_STARSolo_human_heart.sh -g $GENOME_DIR -t $THREADS -1 $FASTQ_R1 -2 $FASTQ_R2 -v $TECHNOLOGY -o "STAR"
    else
...
num_processes=5
find $DIR_TO_PROCESS -maxdepth 1 -type d | xargs -I {} -n 1 -P $num_processes bash -c 'process_dir "$@"' _ {}
