...
def check_donor_balance(metadata, age_col="age_group", min_donors=3):
    '''
    Check if metadata has at least `min_donors` donors for each age group. returns True or False
    '''
    # count donors per group
    donor_counts = metadata.groupby([age_col])["donor_id"].nunique()

    has_young = donor_counts.get("young", 0) >= min_donors
    has_fetal = donor_counts.get("fetal", 0) >= min_donors

    return has_young and has_fetal
...
for cell_type in cell_types:
    ...
    # filter to the postnatal
    filt_metadata = metadata.loc[(metadata.disease_binary != "Y") & metadata.age_group.isin(["fetal", "young"]), ].copy()
    filt_count_matrix = count_matrix.loc[filt_metadata.index, ].copy()

    print( Counter(filt_metadata.age_group) )

    donor_balance_or_not = check_donor_balance(filt_metadata)

    print(f"Donors include at least 3 fetal and 3 young for {cell_type}?: {donor_balance_or_not}")

    # only run DEG analysis if there are at least 3 fetal and young donors with sufficient counts
    if donor_balance_or_not:

        contrasts = [
        ("age_group", "fetal", "young"),
        ("sex", "male", "female"),
        ]
