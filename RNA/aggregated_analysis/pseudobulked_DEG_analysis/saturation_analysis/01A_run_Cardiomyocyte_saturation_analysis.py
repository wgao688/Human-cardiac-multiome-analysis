...
def subsample_and_run_deseq(num_subsample, count_matrix, metadata, contrasts, funcs):
    ...
    deg_counts = {
        ...
        "aging_up_DEGs": len(significant_genes['age_group_old_vs_young']['up']),
        "aging_down_DEGs": len(significant_genes['age_group_old_vs_young']['down']),
        "disease_up_DEGs": len(significant_genes['disease_binary_Y_vs_N']['up']),
        "disease_down_DEGs": len(significant_genes['disease_binary_Y_vs_N']['down']),
...
final_results_df.to_csv("Cardiomyocyte_deg_results_subsampling.csv", index=False)
